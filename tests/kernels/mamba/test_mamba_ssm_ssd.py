...
# 说明：基于一组值生成 1-SS Matrix
# this is the segsum implementation taken from above
def segsum(x):
    """Calculates segment sum."""
    # 说明：x 形状为 (batch, n_heads, num_blocks, block_size)
    T = x.size(-1)
    x = repeat(x, "... d -> ... d e", e=T)
    # 说明：构造一个下三角矩阵的 mask，diagonal=-1 表示不包括对角线
    mask = torch.tril(torch.ones(T, T, device=x.device, dtype=bool), diagonal=-1)
    # 说明：对角线及以上的位置不参与累加，置 0。下三角位置保留原值，进行累加
    x = x.masked_fill(~mask, 0)
    x_segsum = torch.cumsum(x, dim=-2)
    mask = torch.tril(torch.ones(T, T, device=x.device, dtype=bool), diagonal=0)
    # 说明：对角线以下的位置保留累加结果，对角线保持 0 值，对角线以上的位置置 -inf
    x_segsum = x_segsum.masked_fill(~mask, -torch.inf)
    # 说明：x_segsum 形状为 (batch, n_heads, num_blocks, block_size, block_size)，
    # 其中每个 block 内的第 i 行包含了该行之前所有行的累加和（不包括对角线），对角线为 0 值，对角线以上位置为 -inf
    return x_segsum


# 已阅
# 说明：对应 Mamba2 论文中的 6.1 和 6.2 部分
def ssd_minimal_discrete(X, A, B, C, block_len, initial_states=None):
    ...
    # 说明：c 是 block/chunk 数量，l 是 block 大小，h 是 n_heads
    # 在每个 block 内的列维度做 cumsum，得到 A_cumsum 形状为 (batch, n_heads, num_blocks, block_size)，
    A_cumsum = torch.cumsum(A, dim=-1)

    # 1. Compute the output for each intra-chunk (diagonal blocks)
    # 说明：L 形状为 (batch, n_heads, num_blocks, block_size, block_size)，
    # 其中每个 block 内的第 i 行包含了该行之前所有行的乘积（不包括对角线，指数函数相乘，等于指数的和），
    # 对角线为 1，对角线以上位置为 0
    # 说明：对应 Mamba2 论文中的 1-Semiseparable (1-SS) Matrix
    L = torch.exp(segsum(A))
    # 说明：The diagonal blocks are easy to handle, because they are simply self-similar problems of a smaller size.
    # For small chunk lengths Q, this problem is computed more efficiently using the dual quadratic SMA form. 
    # Additionally, the chunks can be computed in parallel. （Mamba2 论文中 Section 6.1 的描述）
    # 对应的 contraction 是 𝑌 = contract(TN, SN, SP, TS → TP)(𝑄, 𝐾, 𝑉, 𝐿)，计算过程参考论文公式 13
    Y_diag = torch.einsum("bclhn,bcshn,bhcls,bcshp->bclhp", C, B, L, X)

    # 2. Compute the state for each intra-chunk
    # (right term of low-rank factorization of off-diagonal blocks; B terms)
    # 说明：A -> [1，2, 3, 4], A_cumsum -> [1, 3, 6, 10], A_cumsum[:, :, :, -1:] -> [10], 
    # A_cumsum[:, :, :, -1:] - A_cumsum -> [9, 7, 4, 0]， decay_states -> [e^9, e^7, e^4, 1]
    # 理解：对于每个 block 内的第 i 行，decay_states 包含了该行与 block 内最后一行之间的衰减乘积
    # （指数函数相乘，等于指数的和）。最后一行没有衰减（乘积为 1），前面每一行根据与最后一行的距离有不同程度的衰减。
    decay_states = torch.exp(A_cumsum[:, :, :, -1:] - A_cumsum)
    # 说明：states 形状为 (batch, num_blocks, n_heads, d_head, d_state)，
    # 计算 the final state per chunk supposing that the initial state (to the chunk) is 0. In other
    # words this computes ℎ_{𝑗Q+Q−1} assuming that 𝑥_{0:𝑗Q} = 0.
    ...
    if initial_states is None:
        # 说明：initial_states 形状为 (batch, 1, n_heads, d_head, d_state)，
        initial_states = torch.zeros_like(states[:, :1])
    # 说明：拼接 initial_states 和每个 chunk 内最后一个位置的状态，
    # 得到 states 形状为 (batch, num_blocks + 1, n_heads, d_head, d_state)，
    states = torch.cat([initial_states, states], dim=1)
    # 说明：A_cumsum[:, :, :, -1] 形状为 (batch, n_heads, num_blocks)，
    # 代表每个 chunk 内最后一个位置的累积值；基于这组值，在前面 pad 后，生成 1-SS Matrix
    # 说明：This is now multiplied by a 1-SS matrix generated by 𝐴_{2Q−1:Q−1}^\times, 𝐴_{3Q−1:2Q−1}^\times, 
    # . . . , 𝐴_{T−1:T−Q−1}^\times.
    # segsum(F.pad(A_cumsum[:, :, :, -1], (1, 0))) 形状为 (batch, n_heads, num_blocks + 1, num_blocks + 1)
