...
# 已阅
# 说明：计算每层的 block 数量（所有层 block 数量相同）
def get_num_blocks(
    ...
    """
    # 说明：available_memory // num_layers 是每层可用的内存，
    # (available_memory // num_layers) // page_size 是每层能放多少个 block
...
# 即除了 block_size 外和特有属性外，其他属性可能不同，进而导致 real_page_size_bytes 不同
...
# 已阅
# 说明：基于以下假设（代码中并没有校验逻辑）：
# 1. block 的 page_size 相同（外部已经通过调用 unify_kv_cache_spec_page_size 方法保证了）
# 2. block_size 相同（逻辑上不一定，但方法是这样假设的）
# 3. physical memory per token per layer 相同（因为 2 的假设成立，所以结合 1，这里的假设也成立）
...
# 说明：上面各点中，1 等于 2,3 的乘积，所以 2,3 相等，1 一定相等；又由于 4 的假设，所有 group 的 layer 数量相同，
# 所以各组 1,4 的乘积也相同
def _get_kv_cache_groups_uniform_page_size(
    ...
    for layers in same_type_layers.values():
        ...
        # 说明：将相邻的层打散到不同的组中；padding layers 并没有相关的实例表示，
        # 使用时可以取所有组中的最大层数，如在 _max_memory_usage_bytes_from_groups 中一样
...
def get_kv_cache_config_from_groups(
    ...
    if len(kv_cache_groups) == 1 and isinstance(
        ...
        kv_cache_tensors = [
            # 说明：每层单独分配 Tensor，因为大小不一致所以不能共享
            # 每层都有 num_blocks 个 block
            KVCacheTensor(
        ...
        # 说明：要求所有 group 的 spec 的 page 大小一致，即只能按照一个页大小分配 block
        page_size = get_uniform_page_size(
            [group.kv_cache_spec for group in kv_cache_groups]
        )
        assert group_size > 0, "group_size must be greater than 0"
        # 说明：根据 KVCacheGroupSpec 的 page_size 和组内层数，计算出每一层有多少 Block；
        # 所有层 block 数量相同
...
# 已阅
# 功能：disable_hybrid_kv_cache_manager 为 True 时调用，将混合 KV cache 规格转换为同一类的实例
def unify_hybrid_kv_cache_specs(kv_cache_spec: dict[str, KVCacheSpec]):
    ...
    # 说明：is_kv_cache_spec_uniform 检查所有 KVCacheSpec 实例是否能够 merge 到一起（完全一致）；
    # UniformTypeKVCacheSpecs.is_uniform_type 检查所有层的 KV cache spec 的 block_size 是否都一致；
    # 是否是同一个类的实例，以及对特定类型 spec 的特定属性值进行一致性检查
...
def get_kv_cache_groups(
    ...
    if vllm_config.scheduler_config.disable_hybrid_kv_cache_manager:
        # 说明：KV cache manager will allocate the same size of KV cache
    ...
    # 3. 所有层的 KV cache block_size 和类型一致（但通用属性字段值和 page_size 可能不完全相同）
    # 4. 所有层的 KV cache page 物理内存大小一致（或能够统一，但 block_size, 类型和规格可能不同）
    ...
    if is_kv_cache_spec_uniform(kv_cache_spec):
        ...
        # each layer.
        # 补充：spec 完全一致（特殊情况是 FullAttentionSpec，会处理 sliding_window 和 attention_chunk_size 有些有值，有些无值的情况）
    ...
    # will raise an error.
    # 说明：KVCacheManager 只能分配一种规格的物理内存，将所有层的 page_size 统一为相同的值；
    # page_size 指一个 block（包含了 block_size 个 token 的 KV cache）的内存大小
...
def generate_scheduler_kv_cache_config(
    ...
    for group in cfg.kv_cache_groups:
        if isinstance(group.kv_cache_spec, UniformTypeKVCacheSpecs):
            ...
            # 说明：用内层 kv_cache_specs 中的第一个 spec 替换
...
# 已阅
# 说明；：根据所有 KV cache 组计算最大内存使用量，考虑了混合模型中的 padding 情况；
def _max_memory_usage_bytes_from_groups(
    ...
    # 说明：特殊情况，每种 spec 的 page_size_bytes 不同
    # UniformTypeKVCacheSpecs special case (single group, per-layer specs)
    if len(kv_cache_groups) == 1 and isinstance(
        kv_cache_groups[0].kv_cache_spec, UniformTypeKVCacheSpecs
    ):
        per_layer_specs = kv_cache_groups[0].kv_cache_spec.kv_cache_specs
        # 说明：这里使用的是内部 per_layer_specs 的 max_memory_usage_bytes 来分别计算内存并求和，
        # 而不是外层 UniformTypeKVCacheSpecs 的 max_memory_usage_bytes；前者是每个 spec 独立做 padding 后求和，
        # 后者是先对 page_size_bytes 求和后整体做 padding
        return sum(
    ...
    # 说明：block_size 都相等，具体见 _get_kv_cache_groups_uniform_page_size 的假设
...
# 已阅
# 说明：根据不同长度对应的最大内存使用量，二分法搜索最大长度，使得内存使用量不超过 available_memory
...
# 已阅
# 说明：根据每个 worker 的 projected_groups_per_worker 和 available_memory 计算最大长度，
# 结合根据模型 HuggingFace 配置推导的初始 max_model_len，确定最终的 max_model_len
def _auto_fit_max_model_len(
    ...
    Args:
        ...
        available_memory: Memory available for KV cache in bytes for each
            worker.
    """
    # 说明：执行 ModelConfig.get_and_verify_max_len 的结果，根据模型 HuggingFace 配置确定的初始值
    original_max = vllm_config.model_config.max_model_len
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
        ...
        # 说明：取所有 worker 中最小的且小于 auto_fit_max 的长度和对应的 avail_mem
...
# 已阅
# 说明：将全局 KV Cache Groups 投影到每个 worker 的 assigned layers 上，得到每个 worker 的 KV Cache Groups；
# worker 的 kv cache groups 数量和全局的保持一致，不存在层的时候 layer_names 为空
def _project_kv_cache_groups_to_worker(
    ...
    for group in global_kv_cache_groups:
        ...
        if worker_layer_names and isinstance(group_spec, UniformTypeKVCacheSpecs):
            # 说明：不同 worker 的 UniformTypeKVCacheSpecs 可能不同，因为每个 worker 的层不同，
            # 所以需要根据 worker_layer_names 中的层来调整 group_spec 中的 kv_cache_specs；
            # 同时，这也导致不同 worker 的 group_spec 的 page_size_bytes 可能不同，
            # 因为它是根据 kv_cache_specs 中的 spec 计算得到的
...
# 已阅
# 说明：根据每个 worker 的可用 memory，分别为每个 worker 生成 kv_cache_config，
...
# 理解：先各自占满内存尽力分配 block，再调整 num_blocks 保证一致，避免内存浪费
def get_kv_cache_configs(
    ...
    # 说明：worker 的 kv cache groups 数量和全局的保持一致，不存在层的时候 layer_names 为空
    ...
    # 说明：创建每个 worker 的 kv_cache_config，基于每个 worker 的 projected_groups_per_worker 和 available_memory；
    kv_cache_configs: list[KVCacheConfig] = []
    ...
    # 说明：每个 worker 包含的 block_size 相同（或假设相同），这里统一 num_blocks，
    # 使所有 worker 能处理相同的 token 长度；
    # 统一 num_blocks 不影响最大模型长度，因为最大模型长度在确定时已经充分考虑了
    # 不同 worker 的 kv cahce group spec 和内存；
    # num_blocks 不同的原因是每个 worker 的 available_memory 和包含的 kv cache group 不同，
    # 所以独立计算出来的 num_blocks 存在差异；
    min_num_blocks = min(
