...
class GPUModelRunner(
    ...
    def __init__(
        ...
        # 在 EngineCore 沿着 executor -> worker -> GPUModelRunner 收集 KV Cache 信息时初始化，
        # 具体见 get_kv_cache_spec 方法
    ...
    def _update_states_after_model_execute(
        ...
        num_accepted_tokens = (
            (
                # 说明：拼接一列 -1，针对所有 token 都不是 -1 的情况
                torch.cat(
                    [
                        output_token_ids,
                        torch.full(
    ...
    # 已阅
    # 说明：根据 kv_cache_config 初始化 attention backend；
    # attention metadata builders 是在调用该方法之后，通过调用 initialize_metadata_builders 来创建的，
    ...
    def initialize_attn_backend(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        # 说明：返回值 tuple，
        # tuple[0]，将每个 kv cache group 组内的层根据对应的 attention backend 进行分组
        # 分组的时候使用 full class name 作为 key（的一部分），返回的时候生成 AttentionGroupKey 实例
        ...
        def get_attn_backends_for_group(
            ...
            # layer.
            # 说明：注意是使用 full class name 去重
            for layer_name in kv_cache_group_spec.layer_names:
                attn_backend = layers[layer_name].get_attn_backend()

                if layer_name in self.kv_sharing_fast_prefill_eligible_layers:
                    # 说明：动态创建 attention backend subclass
        ...
        def create_attn_groups(
            # 说明：AttentionGroupKey -> layer_names
        ...
        # 说明：元素与 kv cache group 一一对应，每个元素是根据 kv cache group 生成的
        # AttentionGroupKey -> layer_names 映射
        attention_backend_maps = []
        # 说明：元素与 kv cache group 一一对应，每个元素表示 kv cache group 组内使用到的
        # 所有 attention backend 类型的集合
        ...
        # 说明：因为在 AttentionMetadataBuilder 的构造函数中可能会用到 cudagraph_mode
        # (e.g. FlashAttentionMetadataBuilder, GDNAttentionMetadataBuilder)，
    ...
    def _check_and_update_cudagraph_mode(
        ...
        if (
            ...
            # 说明：此时 min_cg_support 是 AttentionCGSupport.UNIFORM_BATCH 或
            # AttentionCGSupport.UNIFORM_SINGLE_TOKEN_DECODE，说明 uniform decode 支持
            # cuda graph，但 prefill/mixed 不支持 cuda graph；
            if self.compilation_config.splitting_ops_contain_attention():
                msg += "; setting cudagraph_mode=FULL_AND_PIECEWISE"
                # 说明：piecewise compilation，此时 attention 不在 FX graph 中，
                # 可以使用 cuda graph 来处理非 attention 的部分，所以降级为 (full, piecewise)
                cudagraph_mode = self.compilation_config.cudagraph_mode = (
                    CUDAGraphMode.FULL_AND_PIECEWISE
                )
            else:
                msg += "; setting cudagraph_mode=FULL_DECODE_ONLY"
                # 说明：attention 在 FX graph 中，prefill/mixed 完全无法使用 cuda graph，
                # 所以降级为 (full, none)
                cudagraph_mode = self.compilation_config.cudagraph_mode = (
    ...
    def maybe_add_kv_sharing_layers_to_kv_cache_groups(
        ...
        if self.cache_config.kv_sharing_fast_prefill:
            ...
            # 说明：从后向前，将连续的共享 kv cache 的层放入 
            # kv_sharing_fast_prefill_eligible_layers 集合中，prefill 在执行到这些层时，
            # 可以提前退出，跳过后面的共享层；
            for layer_name in reversed(attn_layers):
                if layer_name in self.shared_kv_cache_layers:
                    self.kv_sharing_fast_prefill_eligible_layers.add(layer_name)
                else:
                    # 说明：遇到不共享 KV cache 的层，就停止继续往前添加，
                    # 只有连续的共享层才能提前退出
                    break
