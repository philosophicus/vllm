...
def _causal_conv1d_fwd_kernel(  # continuous batching
    ...
    if IS_APC_ENABLED:
        ...
        if seq_end_offset == 0:
            # 问题：此时 last_full_block_token_index == sequence_end_index，
            # 那么 last_full_block_token_index - B_size 能保证在序列范围内吗？
    ...
    if chunk_offset == 0:
        ...
        if (
            ...
            # 说明：seqlen >= state_len，将 x 尾部的 state_len 个 token 的数据写入 conv_state（需要填充的最后一个 block）
            ...
            # 说明：state_len > seqlen，需要左移 conv_state，移动长度为 seqlen，然后将 x 的数据写入 conv_state 的尾部
            if load_init_state:
        ...
        # 说明：chunck_offset > 0，一定有 prior token，从 x 中读取
        ...
        if (chunk_offset - 1) < n_block_to_fill:
            # 说明：包含最后一个 block 开始，倒数 n_block_to_fill 个 block，每个 block 最后 state_len 个 token 的起始位置的索引
            ...
            idx_tokens_last = (
                ...
                # 说明：n_block_to_fill - chunk_offset 的范围是 [0, n_block_to_fill)
            ...
            # 说明：conv_state_indices[idx_seq, current_first_index + (chunk_offset - 1)]，conv_state 从头开始填充，
            # 一共 n_block_to_fill 个 block
            # cache_idx
            conv_states_output_coord = tl.load(
                conv_state_indices_ptr
                + idx_seq * stride_cache_indices
...
def _causal_conv1d_update_kernel(
    ...
    if IS_VARLEN:
        ...
        # 调研：state_len 减去 (seqlen - actual_seqlen)，即减去本来不存在的 padding 部分
        ...
        # 说明：x 的 shape 是 [num_tokens, dim]，所以 offset 要乘以 stride_x_token
    ...
    if IS_SPEC_DECODING:
        # 说明：注意，这里的 offset 是 num_accepted_tokens - 1，参考后续的注释，
        # 这是为了读取 [num_accepted_tokens, width - 1 + num_accepted_tokens - 1) 范围内的数据
        # 这里负责处理 acceptance 后的滚动；
        # 此时的 state_len = width - 1 + (seqlen - 1)
    ...
    conv_state_ptrs_source = (
        ...
        + (idx_feats * stride_conv_state_dim)[None, :]
        # 说明：关注这里 speculative decoding 时的偏移为 1，非 speculative decoding 时偏移为 seqlen
    ...
    # 说明：将 shift 后的 conv_state 和 x 进行合并，token 维度上保持 state_len 长度
